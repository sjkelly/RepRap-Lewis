...
module sarrus_top_assembly()
{
  ...
  {
    sarrus_top();
    translate([0,0,washer_thick+screw_head_height])rotate([0,0,30])nut(screw);
    for(i = [0:90:360])
      {
        ...
        rotate([0,0,i])translate([sarrus_top_od/2+sarrus_link_diameter/2,sarrus_link_thick*5/2+washer_thick,sarrus_link_diameter/2])rotate([90,0,0])cap_screw(screw,sarrus_link_thick*4+washer_thick*2+lock_nut_height);
        rotate([0,0,i])translate([sarrus_top_od/2+sarrus_link_diameter/2,sarrus_link_thick*5/2+washer_thick,sarrus_link_diameter/2])rotate([90,0,0])washer(screw);
        rotate([0,0,i])translate([sarrus_top_od/2+sarrus_link_diameter/2,-sarrus_link_thick*3/2,sarrus_link_diameter/2])rotate([90,0,0])flat_washer(screw);
        rotate([0,0,i])translate([sarrus_top_od/2+sarrus_link_diameter/2,-sarrus_link_thick*3/2-washer_thick,sarrus_link_diameter/2])rotate([90,0,0])lock_nut(screw);
      }
    translate([0,lead_nut_diameter/2+screw_diameter/2,sarrus_top_thick-nut_height])nut(screw);
    translate([0,lead_nut_diameter/2+screw_diameter/2,screw_head_height])flat_washer(screw);
    translate([0,lead_nut_diameter/2+screw_diameter/2,screw_head_height])cap_screw(screw,sarrus_top_thick-screw_head_height);
  }
}
