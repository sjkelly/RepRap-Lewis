...
module sarrus_bottom_assembly(){
	union(){
		...
		for(i = [0:90:360]){
			...
			rotate([0,0,i])translate([sarrus_top_od/2+sarrus_link_diameter/2,sarrus_link_thick*5/2+washer_thick,sarrus_link_diameter/2])rotate([90,0,0])screw(sarrus_link_thick*4+washer_thick+lock_nut_height);
			...
			rotate([0,0,i])translate([sarrus_top_od/2+sarrus_link_diameter/2,-sarrus_link_thick*3/2,sarrus_link_diameter/2])rotate([90,0,0])lock_nut();
		...
		translate([motor_flange_diameter/2+nut_height,-washer_diameter/2,sarrus_bottom_thick+nut_diameter])rotate([0,-90,0])lock_nut();	
		translate([motor_flange_diameter/2+nut_height,washer_diameter/2,sarrus_bottom_thick+nut_diameter])rotate([0,-90,0])lock_nut();	
		translate([-motor_flange_diameter/2-nut_height,-washer_diameter/2,sarrus_bottom_thick+nut_diameter])rotate([0,90,0])lock_nut();	
		translate([-motor_flange_diameter/2-nut_height,washer_diameter/2,sarrus_bottom_thick+nut_diameter])rotate([0,90,0])lock_nut();	
	}
