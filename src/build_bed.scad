...
module build_bed_assembly(){
	...
	for(i = [0 : 45 : 360]){
		rotate([0,0,i])translate([(sarrus_top_od+sarrus_top_id)/4,0,build_bed_thick+washer_thick])rotate([180,0,0])screw(build_bed_thick+washer_thick+sarrus_top_thick);
		rotate([0,0,i])translate([(sarrus_top_od+sarrus_top_id)/4,0,build_bed_thick])washer();	
		rotate([0,0,i])translate([(sarrus_top_od+sarrus_top_id)/4,0,-sarrus_top_thick+lock_nut_height])rotate([180,0,30])lock_nut();
